import { setView, toast, progressBar } from '../ui.js';
...
export async function renderStudy(contestId) {
  ...
  setView(`<section class="view"><header class="page-header"><h1>Estudar</h1><p class="muted">Todo o conteúdo abaixo vem da matriz do edital ativo.</p></header>
...
export async function renderLesson(contestId, topicId) {
  ...
  setView(`<article class="reading-view">
    <header class="page-header"><p class="small muted"><a href="#/study">Estudar</a> › ${escapeHtml(topic.label)}</p><h1>${escapeHtml(lesson.title)}</h1></header>
    ...
    <div class="row"><button id="mark-studied" class="button button--primary">Marcar como estudado</button><a class="button" href="#/questions?topic=${encodeURIComponent(topic.id)}">Fazer exercícios</a></div>
    <section class="lesson-section" style="margin-top:24px"><h2>Anotações</h2><textarea id="lesson-note" class="textarea" placeholder="Escreva sua anotação deste tópico..."></textarea><button id="save-note" class="button" style="margin-top:8px">Salvar anotação</button></section>
  </article>`);
