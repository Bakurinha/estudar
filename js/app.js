 ...
 *
 * Responsabilidades: inicializar banco, carregar dados iniciais, escolher concurso
 * ativo, preparar navegação e delegar cada tela ao módulo correspondente.
 */
...
async function boot() {
  try {
    ...
    qs('#route-view').innerHTML = `<div class="notice notice--danger"><strong>Não foi possível iniciar o aplicativo.</strong><p>${escapeHtml(error.message)}</p><p>Recarregue a página. Se o erro persistir, consulte a seção de solução de problemas do README.</p></div>`;
  }
}
...
async function loadContests() {
  const contests = await getAllSmall('contests', 100);
  const saved = (await get('settings', 'activeContestId'))?.value;
  state.activeContestId = contests.some(c => c.id === saved) ? saved : contests[0]?.id;
  const selector = qs('#contest-selector');
  selector.innerHTML = contests.map(c => `<option value="${c.id}">${escapeHtml(c.shortName || c.name)}</option>`).join('');
  selector.value = state.activeContestId || '';
  selector.addEventListener('change', async () => {
    state.activeContestId = selector.value;
    await put('settings', { key: 'activeContestId', value: selector.value });
    await renderRoute();
  });
}
...
function openStudyTimer() {
  const dialog = openDialog(`
    <div class="dialog__content">
      ...
      <div class="grid grid--2">
        <label class="field"><span>Duração</span><select id="timer-minutes" class="select"><option>25</option><option selected>50</option><option>90</option><option value="0">Livre</option></select></label>
        <label class="field"><span>Descrição</span><input id="timer-label" class="input" placeholder="ex.: Português - interpretação"></label>
      </div>
      <div id="timer-display" class="metric" style="margin-top:18px">50:00</div>
    </div>
    <div class="dialog__footer"><button class="button" data-close-dialog>Cancelar</button><button id="timer-start" class="button button--primary">Iniciar</button></div>
  `);
  let timerId;
  let startedAt;
  qs('#timer-minutes', dialog).addEventListener('change', e => qs('#timer-display', dialog).textContent = `${String(e.target.value || 0).padStart(2,'0')}:00`);
  qs('#timer-start', dialog).addEventListener('click', () => {
    ...
    timerId = setInterval(() => {
      ...
      qs('#timer-display', dialog).textContent = `${String(Math.floor(total/60)).padStart(2,'0')}:${String(total%60).padStart(2,'0')}`;
      if (left <= 0) toast('Tempo planejado concluído. Finalize a sessão para registrar.');
    }, 1000);
  }, { once: true });
}
  ...
  if ('serviceWorker' in navigator) {
    window.addEventListener('load', async () => {
      try {
        ...
        /* Faz uma verificação de atualização sem bloquear a abertura do app. */
        ...
        toast('PWA indisponível nesta abertura. O site continua funcionando online.', 'warning');
      }
    });
  }
...
function showUpdateAvailable(registration) {
  const dialog = openDialog(`
    <div class="dialog__content">
      ...
      <p>Uma nova versão do aplicativo já foi baixada. Você pode atualizar agora ou continuar estudando e atualizar depois.</p>
    </div>
